...
float4 MaskPS(PS_INPUT input) : COLOR0
{
	...
	if (hiveVision.r > 0 && hiveVision.r * 100 < maxDistance)
	{
...
float4 FinalCompositePS(PS_INPUT input) : COLOR0
{
    ...
    if (glow.r > 0)
	{
        float4 smoke = tex2D(noiseTextureSampler, input.texCoord * 10 + time * 0.12);
        float opacity = 1 - tex2D( inputTextureSampler2, input.texCoord).a;
		result += glow * smoke * float4(5, 2, 0, 1) * opacity; 
	}
