...
float4 SFXCloakedPS(PS_INPUT input) : COLOR0
{
	...
	const float4 tint = float4(0.25, 0.25, 0.5, 1);
	float intensity = color.r * 0.2126f + color.g * 0.7152f + color.b * 0.0722f;
	return lerp(base, intensity * tint, 0.6);
...
technique SFXCloaked
{
    pass p0
    {
        ...
        ZWriteEnable        = False;
        VertexShader        = compile vs_2_0 SFXBasicVS();
        PixelShader         = compile ps_2_0 SFXCloakedPS();
        CullMode            = None;
    }
}