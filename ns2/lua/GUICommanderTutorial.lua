...
local kBackgroundPos = GUIScale(Vector(32, 200, 0))
local kBackgroundWidth = GUIScale(600)
local kTextPadding = GUIScale(20)
local kFontName = "fonts/AgencyFB_small.fnt"

local kItemSpacing = 4
local kLineHeight = 24

local kSlideInSecs = 0.5

local kIconSize = Vector(32, 32, 0)
...
local function RenderTokens(self)
    ...
    for tokenIndex, token in ipairs(self.tokens) do
        ...
        if token.Type == kTokenType.Text then
           ...
           itemWidth = item:GetTextWidth(token.String)
